...
const ParallaxBackground = () => {
  ...
  const mouseQueueRef = useRef<{ x: number; y: number; px: number; py: number } | null>(null);
  ...
  useEffect(() => {
    const processMouseMove = () => {
      if (mouseQueueRef.current) {
        ...
        const threshold = mobile ? PARALLAX_CONFIG.trail.threshold.mobile : PARALLAX_CONFIG.trail.threshold.desktop;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (!mouseQueueRef.current) {
        mouseQueueRef.current = { x: e.clientX, y: e.clientY, px: e.clientX, py: e.clientY };
      } else {
    ...
    const handleDeviceOrientation = (e: DeviceOrientationEvent) => {
      if (isMobile() && e.gamma !== null && e.beta !== null) {
        ...
        setMousePosition({ x: xNorm * GYROSCOPE_INTENSITY, y: yNorm * GYROSCOPE_INTENSITY });
      }
    };
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (isMobile() && e.touches.length > 0) {
        ...
        setMousePosition({ x: xNorm * TOUCH_INTENSITY, y: yNorm * TOUCH_INTENSITY });
      }
    };
    ...
    const cleanupTrail = () => {
      ...
      if (now - lastUpdateRef.current > CLEANUP_FPS) {
        setCursorTrail((prev) => 
          prev.filter((point) => now - point.timestamp < PARALLAX_CONFIG.trail.duration)
        );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(updateMask, PARALLAX_CONFIG.mask.updateInterval);
      ...
      {cursorTrail.length > 0 && (
        <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 100 }}>
          {cursorTrail.slice(-MAX_CURSOR_TRAIL_DISPLAY).map((point, index) => {
            ...
            const opacity = Math.max(0, 1 - age / PARALLAX_CONFIG.trail.duration) * 0.5;
